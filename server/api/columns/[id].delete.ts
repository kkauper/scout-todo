...
export default defineEventHandler(async (event) => {
  ...
  await db.transaction(async (tx) => {
    const [column] = await tx.select().from(schema.boardColumns).where(eq(schema.boardColumns.id, id)).for('update')
    if (!column) throw createError({ statusCode: 404 })

    const [totalRow] = await tx.select({ count: sql<number>`count(*)::int` }).from(schema.boardColumns)
    ...
    if (tasksInColumn.length > 0) {
      ...
      const [targetColumn] = await tx.select().from(schema.boardColumns).where(eq(schema.boardColumns.id, moveTo))
